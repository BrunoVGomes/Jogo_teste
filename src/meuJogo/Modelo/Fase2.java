 ...
 * emJogo;
 * 
 * public Fase2() { setFocusable(true); setDoubleBuffered(true);
 * 
 * ImageIcon referencia = new ImageIcon("res//Cenario2XY.png"); fundo =
 * referencia.getImage(); altura = fundo.getHeight(null); largura =
 * fundo.getWidth(null);
 * 
 * int novaLargura = 1014; int novaAltura = 631; fundo =
 ...
 * imgPortas.getY(265), null); graficos.drawImage(imgPortas.getImagem(null),
 * imgPortas.getX(385), imgPortas.getY(265), null);
 * graficos.drawImage(imgPortas.getImagem(null), imgPortas.getX(585),
 * imgPortas.getY(265), null); }
 * 
 ...
 * 
 * //player.updateDois(); repaint();
 * 
 * }
 * 
 * public boolean colide(Player player, Portas portas) { Rectangle playerBounds
 * = player.getBounds(); Rectangle portaBounds = portas.getBounds();
 * 
 * if (playerBounds.intersects(portaBounds)) { return true; } return false; }
 * 
 * public void verificarColisao() { Portas portaUm = portas.get(0); Portas
 * portaDois = portas.get(1); Portas portaTres = portas.get(2);
 * 
